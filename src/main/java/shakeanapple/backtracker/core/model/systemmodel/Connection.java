...
public class Connection {
    ...
    private void checkCompatibility(Block from, Block to, List<Variable> variables) {
        List<String> varNames = variables.stream().map(v -> v.getInfo().getName()).collect(Collectors.toList());
        List<String> fromVarNames = from.getOutputsInfo().stream().map(vi -> vi.getName()).collect(Collectors.toList());
